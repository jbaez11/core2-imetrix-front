...
export default function EditarBorrarIntro(){

	/*=============================================
	HOOK para capturar datos
	=============================================*/

	const [intro, editarIntro ] = useState({
	...
	const submitPut = async e => {
		...
		const {imagen, titulo, descripcion, id} = intro;
		...
		if(titulo !== ""){
			...
			const expTitulo = /^([0-9a-zA-Z ]).{1,100}$/;
		...
		const result = await putData(intro);
		...
		if(result.status === 200){
			...
			setTimeout(()=>{window.location.href= "/intro";},3000)
	...
	$(document).on("click", ".editarInputs", function(e){
		...
		$("#editarId").val(data[0]);
		$(".previsualizarImg").attr("src", `${rutaAPI}/mostrar-img-intro/${data[1]}`)
	...
	return(
		...
		<div className="modal fade" id="editarIntro">
			...
			<div className="modal-dialog">
				...
				<div className="modal-content">
					...
					<div className="modal-header">
						<h4 className="modal-title">Editar Customer</h4>
...
const putData = data =>{
	...
	const url = `${rutaAPI}/editar-intro/${data.id}`;
...
/*=============================================
PETICIÓN DELETE intro
=============================================*/

const deleteData = data =>{

	const url = `${rutaAPI}/borrar-intro/${data}`;
	const token = localStorage.getItem("ACCESS_TOKEN");
	const params = {
